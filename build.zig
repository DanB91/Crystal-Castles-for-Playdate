...
pub fn build(b: *std.Build) !void {
    ...
    const build_number = try get_and_increment_build_number(io, b);
    ...
    playdate_mod.addImport("toolbox", toolbox_module);
    playdate_mod.addImport("build_info", build_info_module);
    playdate_mod.addImport("levels.bin", levels_data_module);
...
fn get_and_increment_build_number(io: std.Io, b: *std.Build) !isize {
    const contents = std.Io.Dir.cwd().readFileAlloc(io, BUILD_INFO_FILE_NAME, b.allocator, .unlimited) catch |e| {
        switch (e) {
            error.FileNotFound => {
                try write_build_number(1, io, b);
                return 1;
            },
            else => {
                return e;
    ...
    try write_build_number(build_number + 1, io, b);
    return build_number;
}

fn write_build_number(build_number: isize, io: std.Io, b: *std.Build) !void {
...
fn compile_simulator_binary(
    b: *std.Build,
    toolbox_module: *std.Build.Module,
    build_info_module: *std.Build.Module,
    levels_data_module: *std.Build.Module,
    ...
    mod.addImport("toolbox", toolbox_module);
    mod.addImport("build_info", build_info_module);
    mod.addImport("levels.bin", levels_data_module);
